...
\end{figure}
Per aquisire i dati in streaming dalla piattaforma abbiamo implementato un workflow di Nifi (vedasi \textbf{fig.~\ref{nifi_workflow}}) contenente: 
\begin{itemize} %[noitemsep]
...
Come accennato, con l'API di Kafka per Python abbiamo potuto configurare un consumer processare i dati, creando diversi csv (\href{https://github.com/DBertazioli/NeoMeetup/tree/master/csv/struttura}{link alla repository}), %(FIX: non saprei quanto entrare nei dettagli dei vari csv)
...
Fabri  $<$3 Meetup xD. %to add conclusions
%------------------------------------------------
\phantomsection
