...
distance_member_event (da fixare con variante per venues)
----
match (m:Member)-[r1:WILL_PARTECIPATES]->(e:Event)<-[r:HOSTED_EVENT]-(g:Group)
where g.lon <> 'NONE' and m.lat is not null and r1.response = 'yes'
//where e.id = "260241918"
with point({latitude: toFloat(g.lat),longitude:toFloat(g.lon)}) as p1, point({latitude:toFloat(m.lat), longitude:toFloat(m.long)}) as p2, m.id as id_mem, e.id as id_event
return id_mem, id_event, (max(distance(p1,p2)))/1000 as dist
order by dist desc
----
